...
export default function MovementsPage() {
  ...
  const loadLots = useCallback(async (pid: string) => {
    setLots([]); setLotId("");
    if (!pid) return;
    const { data } = await supabase.from("lots_with_stock")
      .select("id,lot_no,expiry_date,stock_on_hand,is_quarantined")
      .eq("product_id", pid)
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    const n = Number(qty);
    if (!productId || !lotId || !n || n <= 0) return setMsg("Lengkapi produk, lot, dan qty > 0.");
  ...
  return (
    <div className="space-y-4">
      ...
      <form onSubmit={submit} className="rounded-2xl shadow p-4 space-y-3 max-w-2xl border">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <label className="block">
            ...
            <select value={productId} onChange={e => setProductId(e.target.value)} className="w-full border rounded-xl px-3 py-2 mt-1">
              ...
              {products.map(p => <option value={p.id} key={p.id}>{p.sku} — {p.name}</option>)}
            </select>
          </label>
          <label className="block">
            <span className="text-sm">Lot (FEFO default)</span>
